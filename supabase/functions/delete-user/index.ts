...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !supabaseServiceRoleKey) {
    console.error('delete-user: SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY missing')
  ...
  if (deleteError) {
    return json({ error: deleteError.message || 'Échec de la suppression' }, 400, corsHeaders)
  }
  ...
  if (profileDeleteError) {
    console.error('delete-user: profile delete failed', profileDeleteError.message)
  }
